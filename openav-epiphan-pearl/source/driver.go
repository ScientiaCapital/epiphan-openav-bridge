...
// pearlAPIRequest performs an authenticated HTTP request to the Pearl REST API v2.0 and
// returns the raw response body. Shared by pearlAPIGet/pearlAPIPost so the client/auth/
// logging/status-code handling isn't duplicated per verb.
func pearlAPIRequest(socketKey, function, method, endpoint string) ([]byte, error) {
...
func pearlAPIGet(socketKey string, endpoint string) (map[string]interface{}, error) {
	...
	bodyBytes, err := pearlAPIRequest(socketKey, function, "GET", endpoint)
	if err != nil {
		return nil, err
	}
...
func pearlAPIPost(socketKey string, endpoint string) error {
	...
	bodyBytes, err := pearlAPIRequest(socketKey, function, "POST", endpoint)
	if err != nil {
		return err
	}
...
// fetchResultField calls pearlAPIGet(endpoint) and marshals the "result" field, falling back
// to the literal "unknown" if absent — the shared shape of getRecordingStatus/getStorages/getChannels.
func fetchResultField(socketKey, function, endpoint string) (string, error) {
	data, err := pearlAPIGet(socketKey, endpoint)
	if err != nil {
		return "", err
	}
...
func getRecordingStatus(socketKey string) (string, error) {
	framework.Log("getRecordingStatus - called for: " + socketKey)
	return fetchResultField(socketKey, "getRecordingStatus", "/recorders/status")
}

func getStorages(socketKey string) (string, error) {
	framework.Log("getStorages - called for: " + socketKey)
	return fetchResultField(socketKey, "getStorages", "/storages")
}

func getChannels(socketKey string) (string, error) {
	framework.Log("getChannels - called for: " + socketKey)
	return fetchResultField(socketKey, "getChannels", "/channels")
}
